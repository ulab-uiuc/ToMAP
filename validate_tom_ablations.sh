...
# 单个设置
setting="ToMAP"
tested_model_path="/data/ph16/TinyZero/checkpoints/debate/Qwen2.5-3B-Instruct-v10-ToMAP/actor/global_step_200"

# 新增：遍历的tom_style值
tom_styles=("black_random" "black_max" "white")
...
for task in "${tasks[@]}"; do
    ...
    for i in "${!persuadee_model_names[@]}"; do
        ...
        [[ "${persuadee_model_name}" == *"gpt"* ]] && external_persuadee=true

        echo "Using persuadee model: ${persuadee_model_name} with port: ${port}"

        external_persuader=false
        [[ "${setting}" == *"gpt"* ]] && external_persuader=true


        for tom_style in "${tom_styles[@]}"; do
            echo "Using tom_style: ${tom_style}"
            max_width=3

            EXPERIMENT_NAME="${task}/${setting}_${tom_style}/against_${persuadee_model_name}"
            ...
            for trial_num in {0..2}; do
                echo "-----------------------------------------------"
                echo "Starting trial ${trial_num} for task: ${task}, setting: ${setting}, tom_style: ${tom_style}, model: ${persuadee_model_name}"
                echo "-----------------------------------------------"

                seed=17
                [[ "$trial_num" -eq 1 ]] && seed=23
                [[ "$trial_num" -eq 2 ]] && seed=37
                ...
                python3 -m verl.trainer.main_ppo \
                    ...
                    trainer.total_epochs=15 2>&1 | tee logs/validate_${task}_${setting}_${tom_style}_${persuadee_model_name}.log
            done
