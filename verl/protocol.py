...
def pad_dataproto_to_divisor(data: 'DataProto', size_divisor: int):
    ...
    # if len(data) < size_divisor:
    #     pad_size = size_divisor - len(data)
    #     data_padded = DataProto.concat([data] + [data[0]] * pad_size)
...
def list_of_dict_to_dict_of_list(list_of_dict: list[dict]):
    ...
    for key, val in output.items():
        output[key] = np.array(val, dtype=object)
    return output
...
class DataProto:
    ...
    def check_consistency(self):
        ...
        if self.non_tensor_batch is not None:
            for key, val in self.non_tensor_batch.items():
                assert isinstance(val, np.ndarray), f"Err: {key}"
        ...
        if self.batch is not None and len(self.non_tensor_batch) != 0:
            ...
            for key, val in self.non_tensor_batch.items():
                assert isinstance(
                    val, np.ndarray
                ) and val.dtype == object, f'data in the non_tensor_batch must be a numpy.array with dtype=object, err: {key}'
                assert val.shape[
    ...
    def update(self, other: 'DataProto') -> 'DataProto':
    ...
    def chunk(self, chunks: int) -> List['DataProto']:
        ...
        for key, val in self.non_tensor_batch.items():
            assert isinstance(val, np.ndarray), f"only support numpy array, err: {key}"
